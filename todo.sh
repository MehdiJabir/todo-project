...
validate_date() {
    ...
    #checking the overall date format first
...
find() {
    ...
    if [ -z "$title" ]; then
     ...
     title=$(echo "$title" | xargs | sed 's/[]\/$*.^[]/\\&/g')  #escaping regex chars using sed, just in case they mess with the search
...
list(){
    ...
    if ! validate_date "$input_date"; then
            echo "Invalid date provided. Please use the format YYYY-MM-DD."
            return 8
        fi
...
then 
    case $1 in
        ...
        [Ll]ist)
            ...
            if [ -z "$date" ]; then
                date=$(date '+%Y-%m-%d')  # Use today's date if the user presses Enter without input
            fi
            list "$date" ;;
        [Uu]pdate)
